...
+
In all cases, the search bar will appear along with full capabilites to edit and modify the search and the resulting chart or table.

* *PinboardEmbed*
+
To embed ThoughtSpot xref:embed-a-viz.adoc[visualizations] and xref:embed-pinboard.adoc[pinboards]. 
+
The PinboardEmbed component is used to embed both full pinboards with multiple answers, as well as a specific answer on a pinboard without the search bar (referred to as a "visualization"). 
+
There is currently no way to embed an individually saved answer outside of the SearchEmbed, so for a single visualization to be displayed, it must be on a pinboard and embedded uisng the PinboardEmbed with both pinboardId and vizId properties. 
